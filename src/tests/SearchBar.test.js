...
describe('Testa o componente SearchBar', () => {
  it('Testa se existem os inputs radio e o botao de pesquisa', async () => {
  ...
  it('Testa se é possivel pesquisar por ingrediente', async () => {
    const { history } = renderWithRouter(<App />);
    history.push('/meals');
    await waitForElementToBeRemoved(() => screen.getByTestId(emailInput));
  ...
  it('Testa se é possivel pesquisar por nome', async () => {
    const { history } = renderWithRouter(<App />);
    history.push('/meals');
    await waitForElementToBeRemoved(() => screen.getByTestId(emailInput));
    const btnSearch = screen.getByTestId(searchTopBtn);
  ...
  it('Testa se é possivel pesquisar por primeira letra', async () => {
    const { history } = renderWithRouter(<App />);
    history.push('/meals');
    await waitForElementToBeRemoved(() => screen.getByTestId(emailInput));
    const btnSearch = screen.getByTestId(searchTopBtn);
  ...
  it('Testa se aparece o global.alert se for digitado mais de 1 letra', async () => {
    const { history } = renderWithRouter(<App />);
    history.push('/meals');
    await waitForElementToBeRemoved(() => screen.getByTestId(emailInput));
    const btnSearch = screen.getByTestId(searchTopBtn);
  ...
  it('Testa se é possivel possível procurar um ingrediente na rota /drinks', async () => {
    const { history } = renderWithRouter(<App />);
    history.push('/drinks');
    await waitForElementToBeRemoved(() => screen.getByTestId(emailInput));
    const btnSearch = screen.getByTestId(searchTopBtn);
  ...
  it('Testa se ao pesquisar mais de uma letra mostra um global.alert', async () => {
    const { history } = renderWithRouter(<App />);
  ...
  it('Testa procurar uma comida invalida', async () => {
    ...
    await waitFor(() => {
      const input = screen.getByTestId(searchInput);
      userEvent.type(input, 'testealeatorioxdxd');
    });
